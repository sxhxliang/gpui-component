...
//! - A recent items list with metadata and action buttons
...
impl KnowledgeHubStory {
    ...
    fn render_tiles(&self, cx: &Context<Self>) -> impl IntoElement {
        h_flex()
            ...
            .gap_3()
            .flex_wrap()
            .children(self.tiles.iter().map(|tile| self.render_tile(tile, cx)))
    }
    ...
    fn render_recent_list(&self, cx: &Context<Self>) -> impl IntoElement {
        v_flex().w_full().gap_2().children(
            self.recents
                .iter()
                .map(|item| self.render_recent_item(item, cx)),
        )
    }

    fn render_recent_item(&self, item: &RecentItem, cx: &Context<Self>) -> impl IntoElement {
        let theme = cx.theme();
    ...
    fn render_fab(&self, cx: &Context<Self>) -> impl IntoElement {
